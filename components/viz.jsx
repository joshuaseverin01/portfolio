...
// ---------- Ambient dispatch / price ticker (hero) ----------
function AmbientTicker({ motion = 1 }) {
  ...
  useEffect(() => {
    ...
    // Seeded pseudo price series
    const N = 192;
    function priceSeries(phase, amp, noise) {
      const arr = new Array(N);
      for (let i = 0; i < N; i++) {
        const x = i / N;
        let v = Math.sin(x * Math.PI * 4 + phase) * 0.55
    ...
    function draw() {
      ...
      // price series (accent) and dispatch series (signal, thin)
      ...
      const g = ctx.createLinearGradient(0, padY, 0, padY + innerH);
      g.addColorStop(0, hexToRgba(accent, 0.10));
...
function hexToRgba(hex, a) {
  ...
  return `rgba(30,60,45,${a})`;
}

// ---------- Zonal revenue bars (project 02 signature) ----------
// Simplified, legible zone-by-zone bars with hover + ISO switcher.
// Values are grounded in real PJM spread surfaced in Joshua's work.
...
const ISO_DATA = {
  PJM: {
    ...
    spread: "68.9%",
    zones: [
      ["BGE", 49.87], ["COMED", 47.6], ["DAYTON", 45.2], ["AEP", 44.1],
      ["APS", 42.9], ["DEOK", 42.1], ["DOMINION", 41.4], ["DPL", 40.8],
      ["PEPCO", 40.0], ["PPL", 39.3], ["METED", 38.5], ["PENELEC", 37.6],
      ["ATSI", 36.8], ["JCPL", 35.7], ["PSEG", 29.53],
    ],
  },
  ERCOT: {
    ...
    spread: "44.3%",
    zones: [
      ["WEST", 61.2], ["HOUSTON", 58.3], ["SOUTH", 55.6],
      ["CPS", 49.1], ["AUSTIN", 46.2], ["NORTH", 42.4],
    ],
  },
  CAISO: {
    ...
    spread: "34.9%",
    zones: [
      ["SP15", 44.8], ["ZP26", 41.5], ["NP15", 33.2],
    ],
  },
};

function ZonalBars({ defaultIso = "PJM" }) {
  const [iso, setIso] = useState(defaultIso);
  const [hovered, setHovered] = useState(null);
  const d = ISO_DATA[iso];
  const max = Math.max(...d.zones.map(z => z[1]));
    ...
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
        <div>
          ...
          <div className="serif" style={{ fontSize: 28, lineHeight: 1.05 }}>{d.title}</div>
        </div>
        <div className="seg" style={{ minWidth: 220 }}>
          {Object.keys(ISO_DATA).map(k => (
            <button key={k} className={iso === k ? "on" : ""} onClick={() => setIso(k)}>
              {k}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 240px", gap: 28, alignItems: "stretch" }} className="zonal-grid">
        <div>
          {d.zones.map(([name, v], i) => {
            const pct = v / max;
            const isHigh = name === d.high.name;
            const isLow = name === d.low.name;
        ...
        <div style={{ borderLeft: "1px solid var(--line)", paddingLeft: 20, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div>
            ...
            <div className="serif" style={{ fontSize: 56, lineHeight: 1, color: "var(--signal)" }}>{d.spread}</div>
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 6 }}>
              ...
              <span className="mono" style={{ fontSize: 11 }}>Top zone — {d.high.name}</span>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <span className="mono" style={{ fontSize: 11 }}>Bottom — {d.low.name}</span>
            </div>
          </div>
        </div>
      </div>
...
Object.assign(window, { AmbientTicker, ZonalBars, ISO_DATA });
